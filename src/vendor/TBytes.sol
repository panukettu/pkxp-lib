...
using tbytesLib for tbytes global;
...
library tbytesLib {
    ...
    function length(tbytes storage self) internal view returns (uint256 len) {
    ...
    function setCd(tbytes storage self, bytes calldata data) internal {
        /// @solidity memory-safe-assembly
        assembly {
            let len := data.length
    ...
    function set(tbytes storage self, bytes memory data) internal {
        /// @solidity memory-safe-assembly
        assembly {
            let len := mload(data)
    ...
    function agus(tbytes storage self) internal {
        /// @solidity memory-safe-assembly
        assembly {
